...
This repository contains the code and report assets for a cloud computing course design. `swjtu-course-report/` holds the final Typst report source, PDF, and figures. `asst/` keeps the Douban CSV dataset used by the Spark task. `scaffold/` is the main working area:
...
- `addon-c2-mqtt/`: K3s + MQTT edge-computing add-on code.
...
- `python3 -m py_compile scaffold/part1-app/backend/app.py scaffold/part2-spark/*.py scaffold/addon-c2-mqtt/*.py`: check Python syntax.
...
Never commit real cloud credentials, SWR passwords, kubeconfig files, or plaintext Redis passwords. Use Kubernetes Secrets for sensitive values and generate base64 content with `echo -n "value" | base64`. Do not commit offline image archives, extracted vendor packages, or temporary screenshot dumps.

## Course Experiment Guidance

When guiding the course design, keep changes aligned with the final report in `swjtu-course-report/main.typ`. At each experimental action, remind the user what screenshot or terminal output is needed for grading before moving on. Do not invent results; leave `TODO` where the user has not provided evidence yet.
